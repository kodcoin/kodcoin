...
*Setup instructions for a gitian build of Kodcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Kodcoin
...
- [Building Kodcoin](#building-Kodcoin)
...
Clone the git repositories for Kodcoin and gitian and then checkout the Kodcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the Kodcoin repository
...
To build Kodcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the Kodcoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Kodcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/Kodcoin
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/Kodcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit Kodcoin=${COMMIT} --url Kodcoin=${URL} ../Kodcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit Kodcoin=${COMMIT} --url Kodcoin=${URL} ../Kodcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit Kodcoin=${COMMIT} --url Kodcoin=${URL} ../Kodcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/Kodcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/Kodcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/Kodcoin-build.assert
```
