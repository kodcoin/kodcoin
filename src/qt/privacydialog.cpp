...
#include "zKodcoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zKOD ought to be enough for anybody." - Bill Gates, 2017
    ui->zKODpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzKODSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzKOD->setEnabled(false);
        ui->pushButtonMintzKOD->setToolTip(tr("zKOD is currently disabled due to maintenance."));

        ui->pushButtonSpendzKOD->setEnabled(false);
        ui->pushButtonSpendzKOD->setToolTip(tr("zKOD is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zKODpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzKOD_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zKOD is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zKOD...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zKOD in ") +
...
void PrivacyDialog::on_pushButtonSpendzKOD_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zKOD is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zKOD
        sendzKOD();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zKOD
    sendzKOD();
}

void PrivacyDialog::on_pushButtonZKodcoinControl_clicked()
{
    ZKodcoinControlDialog* zKodcoinControl = new ZKodcoinControlDialog(this);
    zKodcoinControl->setModel(walletModel);
    zKodcoinControl->exec();
}

void PrivacyDialog::setZKodcoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzKodcoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzKOD()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Kodcoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zKODpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zKOD is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " KOD </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zKOD</b>";
    ...
    // use mints from zKodcoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZKodcoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZKodcoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zKodcoin selector in case it was used
    ZKodcoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zKodcoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Kodcoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zKodcoin Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zKOD </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zKOD "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zKOD "));
    ui->labelzKODAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzKODSyncStatus->setVisible(fShow);
}
