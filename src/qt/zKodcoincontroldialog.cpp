...
#include "ui_zKodcoincontroldialog.h"
...
std::list<std::string> ZKodcoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZKodcoinControlDialog::listMints;

ZKodcoinControlDialog::ZKodcoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZKodcoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZKodcoinControlDialog::~ZKodcoinControlDialog()
{
    delete ui;
}

void ZKodcoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZKodcoinControlDialog::updateList()
{
...
void ZKodcoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZKodcoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZKodcoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZKodcoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZKodcoinControlDialog::GetSelectedMints()
{
...
void ZKodcoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
