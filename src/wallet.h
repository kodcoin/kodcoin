...
// 7857 = 1*5000 + 1*1000 + 1*500 + 1*100 + 1*50 + 1*10 + 1*5 + 1
static const int ZQ_7857 = 7857;
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 KOD at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zKOD send
enum ZerocoinSpendStatus {
    ZKOD_SPEND_OKAY = 0,                            // No error
    ZKOD_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZKOD_WALLET_LOCKED = 2,                         // Wallet was locked
    ZKOD_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZKOD_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZKOD_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZKOD_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZKOD_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZKOD_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZKOD_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZKOD_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZKOD_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZKOD_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZKOD_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZKOD_SPENT_USED_ZKOD = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZKodcoinAutoBackups(bool fEnabled)
    {
